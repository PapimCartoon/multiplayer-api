class JSON_AS2 {
    ...
    function stringify(arg):String {
        ...
        function error(m) {
            throw new Error('JSONError: '+m + " (error in position="+(at-1)+" containing character=' "+text.charAt(at-1)+" '  problematic text="+text.substring(0,at-1)+")");
        }
        function next() {
            ch = text.charAt(at);
            at += 1;
        ...
        function str() {
            ...
            if (ch == '"' || ch == "'") {
				...
				// if the user didn't user "..." , then the string goes until is only alpha numberic, e.g.,   
				do {
				...
				if (s=='') this.error("Bad string: a string without \"...\" must be only alpha-numeric");
				return s; 
			}
