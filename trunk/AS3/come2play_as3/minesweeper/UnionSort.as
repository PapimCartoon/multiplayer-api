package come2play_as3.minesweeper
{
	...
	public class UnionSort
	{
		...
		public function UnionSort(sortBoard:Array, width:int,height:int)
		{
			...
			for(yPos=0;yPos<width;yPos++)
				for(xPos=0;xPos<height;xPos++) {
					...
					{						
						...
						blankBoxArr.push(BlankBox.create(xPos,yPos,sortBoard[xPos][yPos]));	
					}			
				} 
			...
			trace("All groups:");
			for each (blankBoxArr in blankBoxArrays)
			{
				trace("blankBoxArr=");
				for each(var blank:BlankBox in blankBoxArr)
				{
					trace( "(" + blank.xPos + "/"+ blank.yPos+")"+blank.bordering)
				}
			}
